...
def makeString(state):
    ...
    for n in range(length):
        ...
        if char[0] not in [state.types["lChar"], state.types["uChar"], state.types["sChar"], state.types["Number"], state.types["String"]]:
            raiseQLangException(QLangIllegalCastException("SYNTAX ERROR: tried to convert illegal type %s to string" % state.getKeyFromValue(state.types, char[0])))
...
def add(state):
    ...
    if val1[0] == state.types["Number"] and val2[0] == state.types["Number"]:
        ...
        raiseQLangException(QLangTypeException("SYNTAX ERROR: addition of %s and %s not allowed" % (state.getKeyFromValue(state.types, val1[0]), state.getKeyFromValue(state.types, val2[0]))))
...
def subtract(state):
    ...
    if val1[0] == state.types["Number"] and val2[0] == state.types["Number"]:
        ...
        raiseQLangException(QLangTypeException("SYNTAX ERROR: subtraction of %s and %s not allowed" % (state.getKeyFromValue(state.types, val1[0]), state.getKeyFromValue(state.types, val2[0]))))
...
def multiply(state):
    ...
    if val1[0] == state.types["Number"] and val2[0] == state.types["Number"]:
        ...
        raiseQLangException(QLangTypeException("SYNTAX ERROR: multiplication of %s and %s not allowed" % (state.getKeyFromValue(state.types, val1[0]), state.getKeyFromValue(state.types, val2[0]))))
...
def divide(state):
    ...
    if val1[0] == state.types["Number"] and val2[0] == state.types["Number"]:
        ...
        raiseQLangException(QLangTypeException("SYNTAX ERROR: division of %s and %s not allowed" % (state.getKeyFromValue(state.types, val1[0]), state.getKeyFromValue(state.types, val2[0]))))
